...
async function handleMessage(message: { type: string; payload?: unknown }): Promise<unknown> {
  switch (message.type) {
    ...
    case 'updateSettings': {
      const settings = message.payload as Partial<Parameters<typeof updateSettings>[0]>;
    ...
    case 'startPomodoro': {
      const { workDuration, breakDuration, longBreakDuration } = message.payload as {
        workDuration?: number;
        breakDuration?: number;
        longBreakDuration?: number;
      };
      const result = await startPomodoroMode(workDuration, breakDuration, longBreakDuration);
      return result;
    }
    ...
    case 'addScheduleSlot': {
      const slotData = message.payload as { day: number; startTime: string; endTime: string };
      ...
      if (elapsed >= totalMs) {
        ...
        if (settings.notificationsEnabled) {
          await chrome.notifications.create({
            ...
            message: 'Your focus session completed while you were away. Great work!',
            priority: 1
          });
        }
      }
