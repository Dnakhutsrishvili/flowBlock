...
function App() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const response = await chrome.runtime.sendMessage({ type: 'getTimeRemaining' });
  ...
  const loadStatus = async () => {
    try {
      const response: StatusResponse = await chrome.runtime.sendMessage({ type: 'getStatus' });
      ...
      if (response.currentSession) {
        const timeResponse = await chrome.runtime.sendMessage({ type: 'getTimeRemaining' });
  ...
  const toggleBlocking = async () => {
    const response = await chrome.runtime.sendMessage({ type: 'toggleBlocking' });
    setEnabled(response.enabled);
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {!currentSession ? (
            <>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-gray-700">Start Focus Session</h2>
                <button
                ...
                <div className="space-y-3">
                  <div className="bg-gradient-to-r from-primary/5 to-secondary/5 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-gray-700">Pomodoro Timer</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      ...
                      <div>
                        <label className="text-xs text-gray-500 block mb-1">Break</label>
                        <select
            ...
            <div className="text-center">
              ...
              {pomodoroState?.enabled && (
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium mb-3 ${
                  ...
                  <span>🍅</span>
                  {currentSession.type === 'break' ? 'Break' : `Session ${pomodoroState.currentCycle}`}
              ...
              <div className="relative w-32 h-32 mx-auto mb-4">
                <svg className="w-32 h-32 transform -rotate-90">
                  ...
                  <circle
                    ...
                    cy="64"
                    r="56"
                    stroke={currentSession.type === 'break' ? 'url(#gradientBreak)' : 'url(#gradient)'}
                    strokeWidth="8"
                    fill="none"
                    strokeLinecap="round"
              ...
              <p className="text-sm text-gray-500 mb-4">
                {isPaused ? 'Paused' : currentSession.type === 'break' ? 'Enjoy your break! ☕' : 'Stay focused! 🎯'}
              </p>
              <div className="flex gap-2">
                {pomodoroState?.enabled && (
                  <button
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-lg font-bold text-gray-900">{stats.totalFocusTime}m</p>
          ...
          <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-lg font-bold text-gray-900">{stats.totalBlocks}</p>
        ...
        {/* Quick Add Site */}
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <h2 className="text-sm font-semibold text-gray-700 mb-2">Quick Add Site</h2>
          <div className="flex gap-2">
            <input
              type="text"
              value={newSite}
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-gray-700">Blocked Sites</h2>
            <button
              onClick={openOptions}
          ...
          {blockedSites.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">No sites blocked yet</p>
          ) : (
