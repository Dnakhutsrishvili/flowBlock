...
export async function addBlockedSite(domain: string, category?: string): Promise<BlockedSite> {
...
export async function getWeeklySchedule(): Promise<WeeklySchedule> {
  ...
  return { ...DEFAULT_WEEKLY_SCHEDULE, ...result[STORAGE_KEYS.WEEKLY_SCHEDULE] };
}

export async function updateWeeklySchedule(schedule: Partial<WeeklySchedule>): Promise<WeeklySchedule> {
